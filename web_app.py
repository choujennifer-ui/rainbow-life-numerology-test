...
* {{ box-sizing:border-box; }} body {{ margin:0; color:var(--ink); background:radial-gradient(circle at 7% 0,#eee4f1 0,transparent 27rem),var(--bg); font-family:"Noto Sans TC","Microsoft JhengHei",system-ui,sans-serif; }}
...
@media (max-width:620px) {{ .site-header {{ min-height:62px; padding:0 18px; }} .module-label,.text-action {{ display:none; }} .header-actions {{ margin-left:auto; }} .form-page {{ padding:24px 14px 40px; align-items:start; }} .date-fields {{ grid-template-columns:1.3fr 1fr 1fr; gap:9px; }} .time-fields {{ gap:9px; }} .form-card,.error-card {{ border-radius:18px; }} .report-page {{ padding:30px 12px 44px; }} .calendar-header {{ padding:17px 15px 13px; }} .line-block {{ gap:13px; padding:15px; }} .print-button {{ padding:8px 10px; font-size:12px; }} }}
